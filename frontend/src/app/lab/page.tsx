...
const INSTALL_STEPS = [
  ...
  },
  {
    ...
    code: "pip install -e \".[llm,adapters]\"\n# Optional extras: faster-whisper, piper-tts (multimodal)",
  },
  {
    step: 3,
    title: "Configure",
    code: "# config.yaml ships in the repo — edit inference.ollama.model / paths as needed",
  },
  {
    step: 4,
...
/** POST /query → decompose → graph search → optional probe + waves → LLM synthesis (no browser fallback). */
function PushNodeForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg =
        err instanceof Error ? err.message : "Could not reach the backend. Start the stack (docker compose up) and ensure the API token matches.";
      setLastError(msg);
      toast({
        title: "Query failed",
  ...
  return (
    <div className="ts-card p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-mono font-semibold text-sm text-ts-purple-light">Ask the graph</h3>
        <Badge variant="outline" className="text-[10px]">LLM + graph</Badge>
      </div>

      {lastError && (
        <p className="text-xs text-red-400/90 mb-3 font-mono">{lastError}</p>
      )}

      <p className="text-xs text-muted-foreground mb-4">
        Your prompt is decomposed into concepts, matched against the live graph, and
        optionally explored via a probe node and wave cycles when context is thin; the
        LLM then synthesizes an answer. Requires the Docker backend and Ollama.
      </p>
      ...
      {!submitted ? (
        <form onSubmit={handleSubmit} className="space-y-3">
          <textarea
            ...
            placeholder={
              "e.g. 'What if nodes could merge across sessions?'\n'How does tension relate to emergence?'\n'Explain Redis-backed sharding'"
            }
            className={cn(
        ...
        >
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Answer from the live graph pipeline (decompose → retrieve → explore if needed → LLM).
          </p>
          {lastAnswer && (
          ...
          <Button
            ...
            onClick={() => { setSubmitted(false); setValue(""); setLastAnswer(""); setLastError(""); }}
          >
            Push another
          </Button>
        </motion.div>
